...
import { WrappedTree } from './WrappedTree'
 ...
 * @returns 布局节点树
 */
export function nonLayeredTidyTree<T extends ITreeNode>(
  root: T,
   ...
   * @param parentWt 父节点
   */
  function walk(currentWt: WrappedTree<T>, prevWt?: WrappedTree<T>, parentWt?: WrappedTree<T>) {
    // 水平方向
    const { x = 0, width = 0 } = parentWt || {}
    const { height: curHeight, hGap, vGap, childCount: curChildCount } = currentWt
    const gap = WrappedTree.isHorizontal ? hGap : vGap
    currentWt.x = x + width + gap
    ...
    if (prevWt) {
      if (curChildCount === 0) {
        currentWt.y =
          prevWt.contourBottom + (prevWt.contourBottom - prevWt.y - prevWt.height > vGap ? 0 : vGap)
  ...
  WrappedTree.isHorizontal = isHorizontal
  // do layout
  const wt = WrappedTree.fromNode(root, layoutOption)
  walk(wt)
  return wt
}
